...
public class AuthenticationFilter extends HttpFilter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) 
		...
		// 인증이 필요한 요청 주소들(예: 새 글, 상세보기, ...)에 대해서 로그인 여부를 확인. 
		// --> (1) 로그인이 되어있는 경우, 컨트롤러로 요청을 전달해서 처리.
		
		// --> (2) 로그인이 되어있지 않은 경우, 컨트롤러로 요청을 전달하지 않고 로그인 화면으로 이동.
		...
		// 세션에 로그인 정보(signedInUser)가 있는지 체크.
		...
		if (signedInUser == null) { // 로그인 안 된 상태
			...
			log.debug("로그인 아이디: {}",signedInUser);
			
			// 요청을 계속 처리(--> 요청을 처리하는 서블릿으로 전달)
			chain.doFilter(req, response);
		}
		// ==> sendRedirect() 다음에 doFilter()를 처리하거나, doFilter()를 두 번하면 안 된다. 
