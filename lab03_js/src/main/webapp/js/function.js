...
// JavaScript의 함수는 파라미터의 타입을 검사하지 않음. 
result = add('Hello', 'JavaScript');
console.log(`result = ${result}`);

// JavaScript의 함수는 파라미터의 개수를 검사하지 않음. 
...
// JavaScript의 모든 함수는 arguments 속성(prperty])을 가지고 있음.
function testArgs() {
...
// 숫자 두개를 아규먼트로 전달받아서 뺄셈 결과를 리턴하는 함수.
function minus(x, y) {
    return x - y; 
}
