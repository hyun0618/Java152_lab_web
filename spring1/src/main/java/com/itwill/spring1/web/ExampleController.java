...
public class ExampleController { // --> Controller는 서블릿이 호출하는 메서드를 만드는 것이다.
	...
	public void example1(@RequestParam(name = "username") String username,
		...
		// Integer.parseInt(request.getParamaeter("age")) 호출해서
		// 요청 파라미터 값들을 읽고,
	...
	public String forward() {
		...
		// 포워드 방식의 페이지 이동. ==> 최초 요청 주소가 바뀌지 않음! (--> /test2)
	}
	...
	public String redirect() {
		...
		// 리다이렉트 방식의 페이지 이동. ==> 최초 요청 주소가 리다이렉트 되는 주소로 바뀜! (--> /test)
	}
	
	@GetMapping("/rest1")
	@ResponseBody 
	// 컨트롤러 메서드가 리턴하는 값은 뷰를 찾기 위한 문자열이 아니라, 
	// 클라이언트로 직접 응답되는 데이터(응답패킷의 몸통에 포함되는 데이터)이다.
	
	// 컨트롤러 메서드가 HTTP 응답 본문을 직접 반환하도록 지정할 때 사용한다. 
	// 즉, 이 애너테이션을 사용하면 메서드의 반환 값이 HTTP 응답의 본문으로 설정된다.
	public String rest1() {
		log.debug("rest1()");
