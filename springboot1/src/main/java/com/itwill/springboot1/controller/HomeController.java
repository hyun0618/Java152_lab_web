...
public class HomeController {
	...
	@GetMapping("/") // 주소입력-> getMapping, context path로 들어오는 GET 방식 요청을 처리하는 메서드 
	...
	public void bookDetails1(@RequestParam(name = "id") int id, Model model) {
		// 요청 파라미터 id 값을 찾고, 해당 id를 갖는 Book 객체를 만들기.
		...
		Book book = Book.builder()
				.id(id)
				.title("title-1")
				.author(Author.builder().firstName("first-1").lastName("last-1").build())
				.build();
	...
	public String bookDetails2(@PathVariable(name = "id") int id, Model model) {
		...
		Book book = Book.builder()
				.id(id)
				.title("title-2")
				.author(Author.builder().firstName("first-2").lastName("last-2").build())
				.build();
