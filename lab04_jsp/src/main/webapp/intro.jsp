...
1. Servlet(Server + Applet): WAS에서 실행되는 요청을 처리하고 응답을 보내는 작은 자바 프로그램.
    ...
    (2) 서블릿 동작 방식:
        - 최초 요청 --> 서블릿 객체 생성(생성자 호출) --> 메서드(doGet, doPost) 호출 --> 응답
        - 요청(최초X) --> 생성된 서블릿 객체의 메서드 호출 --> 응답 
    
2. JSP(Java/Jakarta Server Page):
    (1) 서블릿은 순수 자바 코드이기 때문에 HTML을 작성하기가 힘듦.
    (2) HTML 형식의 파일에서 자바 코드들이 실행될 수 있도록 만든 server-side 문법. 
    (3) JSP 동작 원리:
        - 최초 요청 --> jsp 파일을 java 파일로 변환 --> java 코드를 class로 컴파일
            --> 객체 생성 --> 메서드 호출 --> 응답 
        - 요청(최초X) --> 생성된 객체에서 메서드 호출 --> 응답
...
3. JSP의 구성 요소(문법)
    ...
    (4) 스크립트릿(scriptlet): <% ... %>
        jsp 파일이 java 코드로 변환될 때, _jspService(request, response) 메서드 안에 포함되는 자바 코드.
