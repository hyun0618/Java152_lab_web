...
<body>
    ...
    <main>
        ...
        <h2>JSTL, EL을 사용한 리스트</h2>
            ...
            %> 
        <h2>scriptlet, expression을 사용한 테이블</h2>
            <table>
                <thead>
                    <tr>
                        <th>번호</th>
        ...
        <h2>URL 태그</h2>
            <%-- 클릭1 --%>
                <a href="result.jsp?username=gu&est&color=crimson">클릭1</a> <%-- username이 "gu"라고 나옴. --%>
            
            <%-- 클릭2 --%>
                <%-- 질의 문자열의 요청 파라미터 값에 특수기호가 포함될 때 --%>
                <c:url value="result.jsp" var="url">
                    <c:param name="username" value="guest" />
                    <c:param name="color" value="crimson" />
                </c:url>
                <a href="${ url }">클릭2</a> <%-- <c:url value="result.jsp" var="url"> 의 var 값 --%>
